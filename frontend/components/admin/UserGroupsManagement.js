...
export default function UserGroupsManagement() {
    ...
    const handleUpdatePermissions = async (groupId, permissionIds) => {
        try {
            ...
            if (toAdd.length > 0) {
                await api.post(`/user-groups/${groupId}/permissions`, {
                    permissionIds: toAdd
                })
            }
            ...
            if (toRemove.length > 0) {
                await api.delete(`/user-groups/${groupId}/permissions`, {
    ...
    const handleUpdateMembers = async (groupId, userIds) => {
        try {
            ...
            // Thêm members mới
            if (toAdd.length > 0) {
                await api.post(`/user-groups/${groupId}/members`, {
                    userIds: toAdd
                })
            }
            ...
            if (toRemove.length > 0) {
                await api.delete(`/user-groups/${groupId}/members`, {
