...
export default function AssignUsersModal({ evidenceIds, onClose, onSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            for (const evidenceId of evidenceIds) {
                await apiMethods.evidences.assignUsers(evidenceId, {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="sticky top-0 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white p-6 flex items-center justify-between rounded-t-2xl">
                    <div>
                        <h2 className="text-2xl font-bold mb-1">Phân Quyền Nộp File Minh Chứng</h2>
                        <p className="text-indigo-100 text-sm">
                            Chọn người dùng để phân quyền nộp file cho {evidenceIds.length} minh chứng
                        </p>
                    </div>
                    <button
                        onClick={onClose}
