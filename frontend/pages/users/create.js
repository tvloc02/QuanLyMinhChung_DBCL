...
import {
    ...
    Shield, AlertCircle, Check, Eye, EyeOff, X, UserPlus, Home
} from 'lucide-react'
...
const ROLE_OPTIONS = [
    ...
    {
        value: 'reporter',
        label: 'Báo cáo viên', // Vai trò mới
        color: 'from-orange-500 to-amber-500',
        description: 'Nhập minh chứng, viết báo cáo'
    },
    {
        value: 'evaluator',
        label: 'Chuyên gia đánh giá', // Vai trò mới
...
export default function CreateUserPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        fullName: '',
        phoneNumber: '',
        roles: ['evaluator'], // Cập nhật vai trò mặc định
        department: '',
        position: '',
        academicYearAccess: [],
        programAccess: [],
    ...
    useEffect(() => {
        ...
        // Giữ nguyên logic chỉ admin, nhưng sử dụng roles (admin là roles[0] nếu có)
        const canCreate = user?.roles?.includes('admin') || user?.role === 'admin'
    ...
    const handleRoleToggle = (roleValue) => {
        setFormData(prev => {
            ...
            return {
                ...prev,
                roles: newRoles.length > 0 ? newRoles : ['evaluator'] // Cập nhật vai trò mặc định
            }
        })
    }
    ...
    const breadcrumbItems = [
        ...
        { name: 'Quản lý người dùng', href: '/users/users' },
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`relative p-5 border-2 rounded-xl cursor-pointer transition-all ${
                                                isSelected
                                                    ? 'border-blue-500 bg-blue-50 shadow-md' // Màu Xanh Lam
                                        ...
                                        >
                                            <div className="flex items-start space-x-3">
                                                <div className={`flex-shrink-0 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${
                                                    isSelected
                                                        ? 'border-blue-600 bg-blue-600' // Màu Xanh Lam
                                                        : 'border-gray-300'
                                                }`}>
                    ...
                    <div className="flex items-center justify-end gap-4 pt-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl hover:shadow-lg disabled:opacity-50 transition-all font-medium" // Màu Xanh Lam
                        >
